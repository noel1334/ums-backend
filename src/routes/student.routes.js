...
// For admins and ICT staff who can manage student records (Create/Delete)
const canManageStudents = authorize(['admin', 'ictstaff']);

// For admins, ICT staff, and HODs who can view all students or departmental students
...
// Allows student to view their own profile, or admin/HOD/ICT staff to view any student record
const authorizeSelfOrAdminOrHODForStudentView = async (req, res, next) => {
    try {
        ...
        const { user } = req;
        // Check 1: Admin or ICT staff can view any student
        if (user.type === 'admin' || user.type === 'ictstaff') {
            return next();
        }
        ...
        // Check 3: HOD can view departmental students
...
// Allows student to update their own profile, or admin/ICT staff to update any student record
const authorizeSelfOrAdminForStudentUpdate = (req, res, next) => {
    try {
        ...
        // Check 1: Admin or ICT staff can update any student
        if (req.user.type === 'admin' || req.user.type === 'ictstaff') {
            return next();
        }
...
// Route for a lecturer/ICT staff to see students in their courses
...
// Routes for general student list and creation
router.route('/')
    .post(authenticateToken, canManageStudents, uploadImageMiddleware('profileImg', 'single'), StudentController.createStudent)
